...
const calculateATSScore = (resume, _rawText = "") => {
  ...
  if (hasEmail) breakdown.contact += 5;
  else missing.push("Email address");
  if (hasPhone) breakdown.contact += 4;
  else missing.push("Phone number");
  if (hasLinkedin) breakdown.contact += 3;
  else suggestions.push("Add your LinkedIn profile URL to the header");
  if (hasGithub) breakdown.contact += 2;
  else suggestions.push("Add your GitHub profile URL to the header");
  if (hasLocation) breakdown.contact += 1;
  ...
  if (hasSummary) breakdown.sections += 5;
  else missing.push("Professional summary / objective");
  if (hasSkills) breakdown.sections += 5;
  else missing.push("Skills section");
  if (hasExperience) breakdown.sections += 4;
  else suggestions.push("Add work or internship experience");
  if (hasEducation) breakdown.sections += 4;
  else missing.push("Education section");
  if (hasProjects) breakdown.sections += 2;
  else suggestions.push("Add portfolio projects with GitHub links");
  ...
  else if (/data analyst/i.test(inferredRole))
    roleBank = ROLE_KEYWORD_BANKS.data;
  else if (/machine learning/i.test(inferredRole))
    roleBank = ROLE_KEYWORD_BANKS.ml;
  ...
  const matchKw = (kw) => {
    ...
    if (listedSkills.some((s) => s === kwLower || s.includes(kwLower)))
      return true;
    return new RegExp(
  ...
  if (combinedScore >= 70) breakdown.keywords = 30;
    ...
    suggestions.push(
      "Add more role-specific technologies and tools to your skills section",
    );
  }
  ...
  if (expActionVerbs >= 6) breakdown.experience += 5;
    ...
    suggestions.push(
      "Use strong action verbs in experience bullets (built, optimized, led)",
    );

  const expQuantified = (
    expText.match(
      /\b\d+\s*[%+x]|\$[\d,.]+[km]?|\d+\s*(ms|seconds?|hrs?|hours?|users?|requests?)\b/gi,
    ) || []
  ).length;
  ...
  if (expQuantified >= 4) breakdown.experience += 5;
    ...
    suggestions.push(
      "Quantify achievements with numbers (e.g. 40% faster, 10k users)",
    );
  ...
  if (wordCount >= 350) breakdown.readability += 4;
    ...
    suggestions.push(
      "Expand your resume content — aim for 350+ words across all sections",
    );
  ...
  if (totalQuantified >= 5) breakdown.readability += 2;
    ...
    suggestions.push(
      "Add measurable results (e.g. 'reduced load time by 40%', 'served 10k users')",
    );
  ...
  for (const key of Object.keys(breakdown)) {
    breakdownPercent[key] = Math.round(
      (breakdown[key] / BREAKDOWN_MAXES[key]) * 100,
    );
  }
  ...
  "frontend developer": [
    {
      ...
      description:
        "A responsive personal portfolio showcasing projects, skills, and contact information with dark/light mode support.",
      bullets: [
    ...
    },
    {
      ...
      description:
        "A drag-and-drop task management application with priority filtering and due-date reminders.",
      bullets: [
  ...
  "backend developer": [
    {
      ...
      description:
        "A secure REST API for a blogging platform with JWT authentication and role-based access control.",
      bullets: [
    ...
    },
    {
      ...
      description:
        "A high-performance URL shortener with Redis caching, analytics tracking, and Docker deployment.",
      bullets: [
  ...
  "full stack developer": [
    {
      ...
      description:
        "A full-stack e-commerce application with Stripe payments, image uploads, and role-based access control.",
      bullets: [
    ...
    },
    {
      ...
      description:
        "A real-time messaging application with room-based chat, online presence indicators, and persistent chat history.",
      bullets: [
  ...
  "data analyst": [
    {
      ...
      description:
        "An interactive KPI dashboard connected to a SQL Server data warehouse with automated refresh scheduling.",
      bullets: [
    ...
    },
    {
      ...
      description:
        "A machine learning project predicting telecom customer churn with 84% accuracy using Python and scikit-learn.",
      bullets: [
  ...
  "machine learning engineer": [
    {
      ...
      description:
        "A convolutional neural network trained on CIFAR-10 achieving 91% test accuracy, exported and served via Flask.",
      bullets: [
    ...
    },
    {
      ...
      description:
        "A fine-tuned BERT model for binary sentiment classification, containerised and deployed on Google Cloud Run.",
      bullets: [
...
`;
  ...
  try {
    ...
    console.warn(
      "[generateProjectSuggestions] Gemini failed, using fallback:",
      err.message,
    );
    return getProjectFallback(role);
  }
  ...
  try {
    ...
    const buildFallback = async () => {
      ...
      const certificationSuggestions = rawCerts.map((cert) => ({
        ...
        description:
          cert.description ||
          `Recommended based on your ${inferredRole} profile`,
        level:
          cert.level ||
          (inferredLevel === "fresher" ? "Beginner" : "Intermediate"),
        skills: cert.skills || [],
...
`;
            ...
            const { role, experienceLevel } = inferRoleAndExperience(
              mergedResume,
              resumeText,
            );
            ...
            const certificationSuggestions = rawCerts.map((cert) => ({
              ...
              level:
                cert.level ||
                (experienceLevel === "fresher" ? "Beginner" : "Intermediate"),
              skills: cert.skills || [],
...
export const createResume = async (req, res) => {
  try {
    ...
    if (!full_name || !email || !role) {
      return res
        .status(400)
        .json({ error: "full_name, email, and role are required" });
    }
    ...
    else if (/backend|node|api|server/.test(normalizedRole))
      inferredRole = "Backend Developer";
    else if (/data analyst|analyst|bi|sql/.test(normalizedRole))
      inferredRole = "Data Analyst";
    else if (/machine learning|ml|ai|deep learning/.test(normalizedRole))
      inferredRole = "Machine Learning Engineer";
    else if (/full.?stack/.test(normalizedRole))
...
`;
        ...
        if (Array.isArray(parsed) && parsed.length >= 3) {
          ...
          console.warn(
            "[createResume] AI returned fewer than 3 summaries, using hardcoded fallback",
          );
        }
      } catch (err) {
        console.warn(
          "[createResume] Summary generation failed, using hardcoded fallback:",
          err.message,
        );
      }
    }
